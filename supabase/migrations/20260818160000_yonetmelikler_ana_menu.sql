-- Yönetmelikler: İç Kontrol alt menüsü değil, dönem kökünde ana menü (hub)

update public.denetim_donem_menu m
set sira_no = sira_no + 1,
    updated_at = now()
where m.parent_id is null
  and m.sistem_anahtari in ('ic-kontrol-bilgileri', 'insan-kaynaklari-bilgileri');
...
set parent_id = null,
    ...
    sira_no = coalesce((
      select i.sira_no - 1
      from public.denetim_donem_menu i
      where i.donem_id = y.donem_id
        and i.sistem_anahtari = 'ic-kontrol-bilgileri'
      limit 1
    ), 5),
    updated_at = now()
