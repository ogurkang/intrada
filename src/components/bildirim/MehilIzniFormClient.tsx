...
import {
  MEHIL_IZNI_BIRIM,
  MEHIL_IZNI_GUN_SAYISI,
  MEHIL_IZNI_MAKAM,
  mehilIzniBelgeAlanlari,
  mehilIzniBitisTarihiHesapla,
  mehilIzniMetinOlustur,
  mehilIzniTarihFormat,
  mehilIzniTarihGoster,
} from '@/lib/mehil-izni-belge'
import { bildirimTarihDbFromDate, bildirimTarihParse } from '@/lib/bildirim-belge-ortak'
...
export default function MehilIzniFormClient({ personeller, sabitSicil, onKaydet }: Props) {
  ...
  const [mehilBaslangic, setMehilBaslangic] = useState('')
  ...
  const mehilBitisDb = useMemo(() => {
    const bas = bildirimTarihParse(mehilBaslangic)
    if (!bas) return null
    return bildirimTarihDbFromDate(mehilIzniBitisTarihiHesapla(bas))
  }, [mehilBaslangic])

  const mehilBitisGoster = mehilBitisDb ? mehilIzniTarihGoster(mehilBitisDb) : '—'
  ...
  const formHazir =
    ...
    Boolean(nakilTarihi) &&
    Boolean(mehilBaslangic) &&
    Boolean(mehilBitisDb)

  const onizlemeMetin =
    formHazir && mehilBitisDb
      ? mehilIzniMetinOlustur({
          geldigi_kurum: geldigiKurum.trim(),
          nakil_tarihi: nakilTarihi,
          mehil_baslangic_tarihi: mehilBaslangic,
          mehil_bitis_tarihi: mehilBitisDb,
        })
      : null

  const onizlemeAlanlar =
    secili && formHazir && mehilBitisDb
      ? mehilIzniBelgeAlanlari(
          {
  ...
  function gonder() {
    ...
    if (!mehilBaslangic) {
      setHata('Mehil başlangıç tarihi zorunludur.')
      return
    }
    ...
    fd.set('mehil_baslangic_tarihi', mehilBaslangic)
    ...
    startTransition(async () => {
      ...
      router.push('/bildirim/mehil-izni')
      router.refresh()
    })
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-slate-800">Yeni Mehil İzni Bildirimi</h1>
        <p className="text-sm text-slate-600 mt-1 max-w-3xl">
          Yalnızca memur statüsündeki personel seçilebilir. T.C. kimlik numarası personel kaydından
          alınır; mehil bitiş tarihi başlangıca {MEHIL_IZNI_GUN_SAYISI} gün eklenerek hesaplanır.
        </p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Örn. Adalet Bakanlığı İstanbul Adliyesi 14. İcra Müdürlüğü"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                Mehil Başlangıç Tarihi
              </label>
              <input
                type="date"
                value={mehilBaslangic}
                onChange={e => setMehilBaslangic(e.target.value)}
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
          ...
          {mehilBaslangic ? (
            <div className="rounded-lg bg-blue-50 border border-blue-100 px-4 py-3 text-sm text-blue-900">
              ...
              <span className="font-medium">
                {mehilIzniTarihGoster(mehilBaslangic)} – {mehilBitisGoster}
              </span>{' '}
              ({MEHIL_IZNI_GUN_SAYISI} gün)
            </div>
          ) : null}
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          {onizlemeAlanlar && onizlemeMetin ? (
            <div className="text-sm text-slate-800 space-y-4 font-serif leading-relaxed">
              ...
              <div className="text-center font-bold">{MEHIL_IZNI_MAKAM}</div>
              <div className="text-center text-slate-600">{MEHIL_IZNI_BIRIM}</div>
