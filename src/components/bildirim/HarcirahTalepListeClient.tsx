...
import { harcirahTalepAuditDegerGoster, harcirahTalepAuditDiffSatirlari } from '@/lib/harcirah-talep-audit'
import { harcirahTalepTarihGoster } from '@/lib/harcirah-talep-belge'
import type { Tables } from '@/types/database'

export interface HarcirahTalepListeKayit {
  id: number
  sicil_no: string
  ad_soyad: string
  tckn: string | null
...
interface Props {
  kayitlar: HarcirahTalepListeKayit[]
  auditLoglarByRefId: Record<string, Tables<'personel_audit_log'>[]>
}

export default function HarcirahTalepListeClient({ kayitlar, auditLoglarByRefId }: Props) {
  ...
  return (
    <>
          ...
          <table className="w-full text-sm border-collapse min-w-[860px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 font-semibold text-slate-700">Nakil Tarihi</th>
                  ...
                  return (
                    <tr key={k.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-600 tabular-nums whitespace-nowrap">
                        {harcirahTalepTarihGoster(k.nakil_tarihi)}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <SaatGecmisDugmesi
                            ...
                            title="İşlem geçmişi"
                          />
                          <GozDetayLink href={`/bildirim/harcirah-talep/${k.id}`} title="Detay" />
                          <IndirLink
                            href={`/api/bildirim/harcirah-talep/word?id=${k.id}`}
                            title="Word İndir"
                          />
                        </div>
                      </td>
      ...
      <AuditGecmisPanel
        ...
        baslik="Harcırah Talep Bildirimi Geçmişi"
