...
export default function AileDuzenleClient({ kayit, onKaydet }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.hata) setHata(res.hata)
        ...
        router.refresh()
        router.push(`/bildirim/aile/${kayit.id}`)
      }
    })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <input type="hidden" name="sicil_no" value={kayit.sicil_no} />
      <div className="px-3 py-2 bg-slate-50 rounded-lg text-sm">
        <span className="text-slate-500">Personel: </span>
        <span className="font-medium">{kayit.ad_soyad ?? kayit.sicil_no}</span>
      </div>
      ...
      <div className="border border-slate-200 rounded-lg p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input name="esin_ad_soyad" defaultValue={kayit.esin_ad_soyad ?? ''}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-500" />
          </div>
          <div>
            <label className="block text-xs text-slate-600 mb-1">TCKN</label>
            <input name="esin_tckn" defaultValue={kayit.esin_tckn ?? ''} maxLength={11}
          ...
          <div>
            ...
            <select name="is_durumu" defaultValue={kayit.is_durumu ?? ''}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 bg-white">
              <option value="">— Seçiniz —</option>
          ...
          <div>
            ...
            <select name="gelir_durumu" defaultValue={kayit.gelir_durumu ?? ''}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 bg-white">
              <option value="">— Seçiniz —</option>
      ...
      <div className="flex justify-end gap-3 pt-1">
        <Link href={`/bildirim/aile/${kayit.id}`}
          className="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors">İptal</Link>
