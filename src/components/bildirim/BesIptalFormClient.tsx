...
export default function BesIptalFormClient({ personeller, sabitSicil, onKaydet }: Props) {
  ...
  const formHazir = Boolean(seciliSicil && tcknUygun)

  const onizlemeAlanlar =
    secili && formHazir
      ? besIptalBelgeAlanlari(
          { sicil_no: secili.sicil_no, ad_soyad: secili.ad_soyad, tckn: secili.tckn },
          besIptalTarihFormat(),
        )
      : null
  ...
  function gonder() {
    ...
    startTransition(async () => {
      ...
      router.push('/bildirim/bes-iptal')
      router.refresh()
    })
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-slate-800">Yeni BES İptal Talebi</h1>
        <p className="text-sm text-slate-600 mt-1 max-w-3xl">
          Personel seçildiğinde OKS/BES iptal dilekçesi oluşturulur.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Link href="/bildirim/bes-iptal" className="text-sm text-slate-600 hover:text-slate-800">
              İptal
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          {onizlemeAlanlar ? (
            <div className="text-sm text-slate-800 space-y-4 font-serif leading-relaxed">
              ...
              <div className="text-center font-bold">{BES_IPTAL_MAKAM}</div>
              <div className="text-center text-slate-600">{BES_IPTAL_BIRIM}</div>
