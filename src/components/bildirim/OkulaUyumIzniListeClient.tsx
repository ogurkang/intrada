...
} from '@/lib/okula-uyum-izni-audit'
import type { Tables } from '@/types/database'

export interface OkulaUyumIzniListeKayit {
  id: number
  sicil_no: string
  ad_soyad: string
  tckn: string | null
  ...
  return (
    <>
                ...
                kayitlar.map((k, idx) => {
                  ...
                  return (
                    <tr key={k.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-700">{k.ogrenci_ad_soyad}</td>
                      <td className="px-4 py-3 text-slate-700">{k.baslayacagi_sinif}</td>
      ...
      <AuditGecmisPanel
        ...
        baslik="Okula Uyum İzni Talebi Geçmişi"
        diffSatirlari={okulaUyumIzinAuditDiffSatirlari}
        degerGoster={okulaUyumIzinAuditDegerGoster}
      />
    </>
  )
}
