...
import {
  kesintiDonemAuditDiffSatirlari,
  kesintiDonemAuditDegerGoster,
} from '@/lib/kesinti-donem-audit'
import { broadcastIntradaRefresh } from '@/lib/intrada-tab-sync'

export type PerformansDonemSatir = {
  id: number
  yil: number
  sira_no: string | null
  ...
  bitis_tarihi: string
  durum: 'Açık' | 'Kapalı' | 'Yayınlandı'
  degerlendirme_sayisi?: number
}

interface Props {
  donemler: PerformansDonemSatir[]
  onEkle: (fd: FormData) => Promise<{ hata?: string; id?: number }>
...
function sonrakiSiraNo(donemler: PerformansDonemSatir[], yil: number): string {
  let max = 0
  for (const d of donemler) {
    if (d.yil !== yil) continue
    const m = String(d.sira_no ?? '').match(/\/(\d+)\s*$/)
...
function DonemForm({
  ...
  open: boolean
  onClose: () => void
  secili: PerformansDonemSatir | null
  onSubmit: (fd: FormData) => Promise<void>
  isPending: boolean
  hata: string | null
  onerilenSiraNo: string
}) {
  const d = secili
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="text"
              readOnly
              value={yeni ? onerilenSiraNo : (d?.sira_no ?? '—')}
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-600"
            />
            {yeni && (
              <p className="text-xs text-slate-400 mt-1">Kayıt sırasında otomatik atanır.</p>
            )}
          </div>
        </div>
        <div>
          ...
          <input
            ...
            placeholder={`${buYil} Performans Dönemi`}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
          />
        </div>
        <div className="grid grid-cols-2 gap-4">
...
export default function PerformansDonemClient({
  donemler,
  onEkle,
  onGuncelle,
  onKapat,
  ...
  const [durumFiltre, setDurumFiltre] = useState<'Tümü' | 'Açık' | 'Kapalı' | 'Yayınlandı'>('Tümü')
  const [formAcik, setFormAcik] = useState(false)
  const [seciliDonem, setSeciliDonem] = useState<PerformansDonemSatir | null>(null)
  ...
  const onerilenSiraNo = useMemo(() => sonrakiSiraNo(donemler, formYil), [donemler, formYil])
  const acikSayisi = donemler.filter(d => d.durum === 'Açık').length

  function yeniEkleAc() {
    setSeciliDonem(null)
    setFormYil(yilFiltre)
    setSunuciHata(null)
    setFormAcik(true)
  }
  function duzenleAc(d: PerformansDonemSatir) {
    setSeciliDonem(d)
    setFormYil(d.yil)
    setSunuciHata(null)
    setFormAcik(true)
  ...
  async function handleSubmit(fd: FormData) {
    ...
    startTransition(async () => {
      ...
      if (res.hata) setSunuciHata(res.hata)
        ...
        kapat()
        broadcastIntradaRefresh('performans')
        router.refresh()
      }
    })
  }
  ...
  function durumRenk(durum: string) {
    if (durum === 'Açık') return 'bg-green-100 text-green-700'
    if (durum === 'Yayınlandı') return 'bg-indigo-100 text-indigo-700'
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        {(['Tümü', 'Açık', 'Kapalı', 'Yayınlandı'] as const).map(d => (
          <button
            key={d}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-center px-4 py-3 font-semibold text-slate-600 w-16">Kayıt</th>
              <th className="text-center px-4 py-3 font-semibold text-slate-600 w-44">
            ...
            {filtreli.map(d => (
              <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3 font-medium text-slate-800">
                  {d.donem_adi ?? `${d.yil} Dönemi`}
                </td>
                <td className="px-4 py-3 text-center text-xs text-slate-500 tabular-nums">
                  {tarih(d.baslangic_tarihi)}
                </td>
                <td className="px-4 py-3 text-center text-xs text-slate-500 tabular-nums">
                  {tarih(d.bitis_tarihi)}
                </td>
                <td className="px-4 py-3 text-center">
                  <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${durumRenk(d.durum)}`}>
                    {d.durum}
                  </span>
                </td>
                <td className="px-4 py-3 text-center text-xs text-slate-500 tabular-nums">
                  {d.degerlendirme_sayisi ?? 0}
                </td>
      ...
      <DonemForm
        open={formAcik && !saltOkunur}
        onClose={kapat}
        secili={seciliDonem}
        onSubmit={handleSubmit}
        isPending={isPending}
        hata={sunuciHata}
        onerilenSiraNo={onerilenSiraNo}
      />
      ...
      <AuditGecmisPanel
        ...
        baslik="Performans — Dönem Geçmişi"
