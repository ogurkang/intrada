...
import {
  ...
  type IzyDetayData,
  type IzyDetayIzin,
} from '@/app/(dashboard)/kesintiler/izy/[donem_id]/actions'
import {
  kesintimHesapla,
...
function izinSirala(izinler: IzyDetayIzin[], sutun: SortSutun, yon: SortYon): IzyDetayIzin[] {
...
export default function IzyDetayClient({ donemId }: Props) {
  const [data, setData]      = useState<IzyDetayData | null>(null)
  ...
  async function yukle() {
    setYukleniyor(true)
    setHata(null)
    const res = await izyDetayYukle(donemId)
    setYukleniyor(false)
    if ('hata' in res) {
      setHata(res.hata)
      setData(null)
  ...
  function kaydet() {
    ...
    startTransition(async () => {
      const res = await izySecimleriKaydet(donemId, siraNoList)
  ...
  function hepsiniIptal() {
    ...
    startTransition(async () => {
      const res = await izySecimleriKaydet(donemId, [])
      if (res.hata) setHata(res.hata)
      else yukle()
    })
  }
  ...
  async function excelIndir(tip: 'ozet' | 'detay') {
    try {
      const res = await fetch(`/api/kesintiler/izy/excel?donem_id=${donemId}&tip=${tip}`)
      if (!res.ok) {
      ...
      a.href = url
      a.download = `Izinli_Zabitalar_${data?.donem.donem_adi ?? 'Donem'}${tip === 'ozet' ? '_Ozet' : '_Detay'}.xlsx`.replace(/[:\*\?\/\\]/g, ' ')
      a.click()
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <IzinTablo izinler={data.islenecek} onSolaAl={solaAl} yon="islenecek" sortable tip="İzinli Zabıta" />
      </div>
