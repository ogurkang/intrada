...
import {
  ...
  type IvyDetayData,
  type IvyDetayIzin,
} from '@/app/(dashboard)/kesintiler/ivy/[donem_id]/actions'
import {
  kesintimHesapla,
...
function izinSirala(izinler: IvyDetayIzin[], sutun: SortSutun, yon: SortYon): IvyDetayIzin[] {
...
export default function IvyDetayClient({ donemId }: Props) {
  const [data, setData]      = useState<IvyDetayData | null>(null)
  ...
  async function yukle() {
    setYukleniyor(true)
    setHata(null)
    const res = await ivyDetayYukle(donemId)
    setYukleniyor(false)
    if ('hata' in res) {
      setHata(res.hata)
      setData(null)
  ...
  function kaydet() {
    ...
    startTransition(async () => {
      const res = await ivySecimleriKaydet(donemId, siraNoList)
  ...
  async function excelIndir(tip: 'ozet' | 'detay') {
    try {
      const res = await fetch(`/api/kesintiler/ivy/excel?donem_id=${donemId}&tip=${tip}`)
      if (!res.ok) {
      ...
      a.href = url
      a.download = `Izinli_Vekiller_${data?.donem.donem_adi ?? 'Donem'}${tip === 'ozet' ? '_Ozet' : '_Detay'}.xlsx`.replace(/[:\*\?\/\\]/g, ' ')
      a.click()
  ...
  function hepsiniIptal() {
    ...
    startTransition(async () => {
      const res = await ivySecimleriKaydet(donemId, [])
      if (res.hata) setHata(res.hata)
      else yukle()
    })
  }
  ...
  async function ozetOnizle() {
    ...
    if (siraNoList.length > 0) {
      ...
      if (siciller.length > 0) {
        ...
        // Vekil unvanı: kadro_hareketleri'nde vekil olarak yer aldığı pozisyonun unvanı (gorev_unvani / kadro_unvani)
        ...
        ;(kadroRaw ?? []).forEach(k => {
          ...
          if (sicil && !unvanMap[sicil]) {
            unvanMap[sicil] = (k.gorev_unvani ?? k.kadro_unvani ?? '').trim()
          }
        })
      }
      izinler = (izinRaw ?? [])
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <IzinTablo izinler={data.islenecek} onSolaAl={solaAl} yon="islenecek" sortable tip="İzinli Vekil" />
      </div>
