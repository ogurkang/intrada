...
interface Props {
  ...
  donemAdi: string
  amaclar: SpAmac[]
  onEkle: (donemId: number, fd: FormData) => Promise<{ hata?: string }>
  onGuncelle: (id: number, donemId: number, fd: FormData) => Promise<{ hata?: string }>
}

export default function StratejikPlanAmacClient({ donemId, donemAdi, amaclar, onEkle, onGuncelle }: Props) {
  const router = useRouter()
  const [formAcik, setFormAcik] = useState(false)
  const [secili, setSecili] = useState<SpAmac | null>(null)
  ...
  function kaydet(fd: FormData) {
    ...
    startTransition(async () => {
      const res = secili ? await onGuncelle(secili.id, donemId, fd) : await onEkle(donemId, fd)
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">{donemAdi}</h1>
          <p className="text-sm text-slate-500 mt-1">Amaçları seçerek detay ekranında hedefleri yönetebilirsiniz.</p>
        </div>
          ...
          <table className="w-full text-sm min-w-[980px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-4 py-3 text-left">Amaç Adı</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {amaclar.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-slate-500">Bu dönem için amaç kaydı bulunamadı.</td>
                </tr>
              ) : (
                amaclar.map((a, i) => (
                  <tr key={a.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 font-medium text-slate-800">
                      <Link href={`/stratejik-yonetim/stratejik-plan/islemler/${donemId}/${a.id}`} className="hover:underline">
                        {a.amac_adi}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-slate-600">Bu amaca bağlı {a.hedef_sayisi} hedef bulunmaktadır.</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <Link href={`/stratejik-yonetim/stratejik-plan/islemler/${donemId}/${a.id}`} className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors" title="Detay">
      ...
      <Modal open={formAcik} onClose={() => setFormAcik(false)} title={secili ? 'Amaç Düzenle' : 'Amaç Ekle'} size="sm">
        <form onSubmit={async e => { e.preventDefault(); await kaydet(new FormData(e.currentTarget)) }} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Amaç Adı *</label>
            <input name="amac_adi" required defaultValue={secili?.amac_adi ?? ''} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" />
          </div>
