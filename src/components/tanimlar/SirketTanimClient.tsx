...
import type { SirketKayit } from '@/app/(dashboard)/tanimlar/sirket/page'

export type SirketYerleskeEsleme = {
...
export default function SirketTanimClient({
  data,
  yerleskeSecenekleri,
  onAdd,
  onUpdate,
  ...
  const [secili, setSecili] = useState<SirketKayit | null>(null)
  ...
  function duzenle(item: SirketKayit) {
    setSecili(item)
    setSunuciHata(null)
    setModalAcik(true)
  }
  ...
  function handleToggle(item: SirketKayit) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Şirketler</h1>
        {!saltOkunur && (
          <button
            type="button"
            onClick={yeniEkle}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-left px-5 py-3 font-semibold text-slate-600">Şirket Adı</th>
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {data.map((item, i) => (
              <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-5 py-3 font-medium text-slate-800">{item.sirket_adi}</td>
      ...
      <Modal
        open={modalAcik}
        onClose={kapat}
        title={secili ? 'Şirket Düzenle' : 'Yeni Şirket Ekle'}
        size="md"
      >
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              type="text"
              required
              defaultValue={secili?.sirket_adi ?? ''}
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mb-2">
              Her yerleşke için ayrı İç/Dış konumu seçin. Aynı şirket farklı yerleşkelerde farklı konumda olabilir.
            </p>
