...
import {
  terfiDonemAuditDiffSatirlari,
  terfiDonemAuditDegerGoster,
...
function islemBadgeClass(islem: string): string {
  ...
  if (i.includes('kapat')) return 'bg-slate-100 text-slate-700'
  if (i.includes('aç')) return 'bg-emerald-100 text-emerald-700'
  if (i.includes('geri al')) return 'bg-rose-100 text-rose-700'
  if (i.includes('ettir')) return 'bg-fuchsia-100 text-fuchsia-700'
...
export default function TerfiDonemGecmisPanel({ acik, onKapat, auditLoglar, baslik }: Props) {
  ...
  const filtreli = useMemo(() => {
    ...
    return auditLoglar.filter(log => {
      ...
      const sicil = String(log.sicil_no ?? '').toLocaleLowerCase('tr-TR')
      return actor.includes(q) || ozet.includes(q) || islem.includes(q) || sicil.includes(q)
    })
  }, [auditLoglar, search])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] bg-white rounded-xl border border-slate-200 shadow-xl flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{baslik ?? 'Dönem İşlem Geçmişi'}</h2>
            <p className="text-xs text-slate-500 mt-1">
              Dönem tanımı, aç/kapa ve terfi ettir/geri al işlemleri. Satıra tıklayarak detay görebilirsiniz.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
          <input
            ...
            placeholder="Özet, sicil veya kullanıcı ara…"
            ...
            <tbody className="divide-y divide-slate-100">
              {filtreli.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-slate-400">
                    {auditLoglar.length === 0
                      ? 'Bu dönem için henüz kayıtlı işlem yok.'
                ...
                filtreli.map(log => {
                  ...
                  const diffSatirlari = terfiDonemAuditDiffSatirlari(log.onceki, log.sonraki)
                  return (
                    <Fragment key={log.id}>
                      <tr
                                    ...
                                    {diffSatirlari.map(d => (
                                      <tr key={d.alan}>
                                        ...
                                        <td className="px-3 py-2 text-slate-600">
                                          {terfiDonemAuditDegerGoster(d.alan, d.onceki)}
                                        </td>
                                        <td className="px-3 py-2 text-slate-800">
                                          {terfiDonemAuditDegerGoster(d.alan, d.sonraki)}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
