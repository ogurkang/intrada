...
import { firmaAuditDegerGoster, firmaAuditDiffSatirlari } from '@/lib/firma-audit'
...
function islemBadgeClass(islem: string): string {
  ...
  if (i.includes('sil')) return 'bg-rose-100 text-rose-700'
  if (i.includes('ekle')) return 'bg-emerald-100 text-emerald-700'
  if (i.includes('güncelle') || i.includes('yerleşke')) return 'bg-indigo-100 text-indigo-700'
  return 'bg-slate-100 text-slate-700'
}

export default function FirmaGecmisPanel({ acik, onKapat, auditLoglar, yerleskeMap }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] bg-white rounded-xl border border-slate-200 shadow-xl flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">ADABEL Değişiklik Geçmişi</h2>
            <p className="text-xs text-slate-500 mt-1">
              Bu personel kaydındaki tüm işlemler. Satıra tıklayarak alan bazlı eski/yeni değerleri görebilirsiniz.
            </p>
          </div>
          <button
            type="button"
            ...
            <tbody className="divide-y divide-slate-100">
              {filtreli.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-slate-400">
                    {auditLoglar.length === 0
                      ? 'Bu personel için henüz kayıtlı değişiklik yok.'
                ...
                filtreli.map(log => {
                  ...
                  const diffSatirlari = firmaAuditDiffSatirlari(log.onceki, log.sonraki)
                  return (
                    <Fragment key={log.id}>
                      <tr
                                    ...
                                    {diffSatirlari.map(d => (
                                      <tr key={d.alan}>
                                        ...
                                        <td className="px-3 py-2 text-slate-600">
                                          {firmaAuditDegerGoster(d.alan, d.onceki, yerleskeMap)}
                                        </td>
                                        <td className="px-3 py-2 text-slate-800">
                                          {firmaAuditDegerGoster(d.alan, d.sonraki, yerleskeMap)}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
