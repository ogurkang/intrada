...
import { terfiAuditDegerGoster, terfiAuditDiffSatirlari } from '@/lib/terfi-audit'
...
function islemBadgeClass(islem: string): string {
  ...
  if (i.includes('sil') || i.includes('geri al')) return 'bg-rose-100 text-rose-700'
  if (i.includes('ekle')) return 'bg-emerald-100 text-emerald-700'
  if (i.includes('ettir')) return 'bg-fuchsia-100 text-fuchsia-700'
  if (i.includes('güncelle') || i.includes('senkron')) return 'bg-indigo-100 text-indigo-700'
  return 'bg-slate-100 text-slate-700'
}

export default function TerfiGecmisPanel({ acik, onKapat, auditLoglar, baslik }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] bg-white rounded-xl border border-slate-200 shadow-xl flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{baslik ?? 'Terfi Değişiklik Geçmişi'}</h2>
            <p className="text-xs text-slate-500 mt-1">
              Terfi kaydındaki tüm işlemler. Satıra tıklayarak alan bazlı eski/yeni değerleri görebilirsiniz.
            </p>
          </div>
          <button
            type="button"
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-slate-400">
                    {auditLoglar.length === 0
                      ? 'Bu terfi kaydı için henüz değişiklik logu yok.'
                                    ...
                                    {diffSatirlari.map(d => (
                                      <tr key={d.alan}>
                                        ...
                                        <td className="px-3 py-2 text-slate-600">
                                          {terfiAuditDegerGoster(d.alan, d.onceki)}
                                        </td>
                                        <td className="px-3 py-2 text-slate-800">
                                          {terfiAuditDegerGoster(d.alan, d.sonraki)}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
