...
import { izinAuditDegerGoster, izinAuditDiffSatirlari } from '@/lib/izin-audit'
...
export default function IzinGecmisPanel({ acik, onKapat, auditLoglar, baslik }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] bg-white rounded-xl border border-slate-200 shadow-xl flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{baslik ?? 'İzin Hareketi Geçmişi'}</h2>
            <p className="text-xs text-slate-500 mt-1">
              Bu izin kaydındaki tüm işlemler. Satıra tıklayarak alan bazlı eski/yeni değerleri görebilirsiniz.
            </p>
          </div>
          <button
            type="button"
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-slate-400">
                    {auditLoglar.length === 0
                      ? 'Bu izin kaydı için henüz değişiklik kaydı yok.'
                                    ...
                                    {diffSatirlari.map(d => (
                                      <tr key={d.alan}>
                                        ...
                                        <td className="px-3 py-2 text-slate-600">
                                          {izinAuditDegerGoster(d.alan, d.onceki)}
                                        </td>
                                        <td className="px-3 py-2 text-slate-800">
                                          {izinAuditDegerGoster(d.alan, d.sonraki)}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
