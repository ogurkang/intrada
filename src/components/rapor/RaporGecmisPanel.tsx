...
function islemBadgeClass(islem: string): string {
  ...
  if (i.includes('excel')) return 'bg-emerald-100 text-emerald-700'
  if (i.includes('kapsam')) return 'bg-indigo-100 text-indigo-700'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] bg-white rounded-xl border border-slate-200 shadow-xl flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{baslik ?? 'Rapor Kapsam Geçmişi'}</h2>
            <p className="text-xs text-slate-500 mt-1">
              Rapor içeriği değil; liste seçimi ve excel aralığı gibi kapsam değişiklikleri kaydedilir.
            </p>
          </div>
          <button
            type="button"
                                    ...
                                    {diffSatirlari.map(d => (
                                      <tr key={d.alan}>
                                        ...
                                        <td className="px-3 py-2 text-slate-600">
                                          {raporKapsamDegerGoster(d.alan, d.onceki)}
                                        </td>
                                        <td className="px-3 py-2 text-slate-800">
                                          {raporKapsamDegerGoster(d.alan, d.sonraki)}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
