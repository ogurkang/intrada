...
export async function GET(request: NextRequest) {
  ...
  if (tip === 'ozet') {
    ...
    rows.push(mergeSatir('İzinli Vekiller', colCount))
    mergeRows.push(rows.length - 1)
    rows.push(mergeSatir(donemMetin, colCount))
    mergeRows.push(rows.length - 1)
  ...
  const safeName = (`Izinli_Vekiller_${donem.donem_adi ?? 'Donem'}`).replace(/[:\*\?\/\\]/g, ' ').trim().substring(0, 90) || 'Izinli_Vekiller_Donem'
