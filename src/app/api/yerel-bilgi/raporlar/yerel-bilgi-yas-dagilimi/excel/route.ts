...
export async function GET(req: Request) {
  try {
    ...
    const [
      ...
      fetchAllFirmaCalisanlar(supabase, 'id, ad_soyad, cinsiyet, kuruma_giris_tarihi, ayrilis_tarihi, dogum_tarihi'),
      supabase
    ...
    const rows: (string | number)[][] = [
      padRow(COL_LAST + 1, ['Yerel Bilgi İçin Yaş Raporu']),
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Yerel Bilgi Yaş')
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })
    const filename = `Yerel_Bilgi_Yas_Dagilimi_${yil}_${label}.xlsx`
    const encodedFilename = encodeURIComponent(filename)

    return new NextResponse(buf, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Yerel_Bilgi_Yas_Dagilimi.xlsx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('YEREL_BILGI_YAS_DAGILIMI_EXCEL_HATA', err)
