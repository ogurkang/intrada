...
import { harcirahTalepImzaTablosu } from '@/lib/bildirim-word-imza'
import {
...
export async function GET(req: Request) {
  try {
    ...
    const { data: kayit } = await (supabase as any)
      ...
      .select('id, sicil_no, ad_soyad, tckn, adres, geldigi_kurum, nakil_tarihi, created_at')
      .eq('id', id)
      .maybeSingle()
    ...
    const tarih = kayit.created_at
      ? harcirahTalepTarihFormat(new Date(kayit.created_at))
      : harcirahTalepTarihFormat()

    const alanlar = harcirahTalepBelgeAlanlari(kayit, tarih)
      ...
      sections: [
        {
          ...
          children: [
            ...
            new Paragraph({
              ...
              children: [run(HARCIRAH_TALEP_MAKAM, { bold: true })],
            }),
            new Paragraph({
              alignment: AlignmentType.CENTER,
              spacing: { after: 600 },
              children: [run(HARCIRAH_TALEP_BIRIM)],
            }),
            new Paragraph({
            ...
            bosSatir(),
            bosSatir(),
            harcirahTalepImzaTablosu(alanlar.adres, alanlar.tckn, alanlar.ad_soyad),
          ],
        },
      ],
    })
    ...
    const filename = `Harcirah_Talep_Bildirimi_${safeKey}.docx`
    ...
    return new NextResponse(new Uint8Array(buf), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Harcirah_Talep_Bildirimi.docx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('HARCIRAH_TALEP_WORD_HATA', err)
