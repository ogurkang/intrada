...
export async function GET(req: Request) {
  try {
    ...
    const { data: kayit } = await (supabase as any)
      ...
      .select(
        'id, sicil_no, ad_soyad, tckn, unvan, mudurluk, baslangic_tarihi, bitis_tarihi, created_at',
      )
      .eq('id', id)
      .maybeSingle()
    ...
    const tarih = kayit.created_at
      ? aylikIzinTarihFormat(new Date(kayit.created_at))
      : aylikIzinTarihFormat()

    const alanlar = aylikIzinBelgeAlanlari(kayit, tarih)
      ...
      sections: [
        {
          ...
          children: [
            ...
            new Paragraph({
              ...
              children: [run(AYLIK_IZIN_MAKAM, { bold: true })],
            }),
            new Paragraph({
              alignment: AlignmentType.CENTER,
              spacing: { after: 600 },
              children: [run(AYLIK_IZIN_BIRIM)],
            }),
            new Paragraph({
    ...
    console.error('AYLIK_IZIN_WORD_HATA', err)
