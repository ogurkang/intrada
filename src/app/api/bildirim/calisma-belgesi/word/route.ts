...
export async function GET(req: Request) {
  try {
    ...
    const { data: kayit } = await (supabase as any)
      ...
      .select('id, sicil_no, ad_soyad, tckn, unvan, mudurluk, created_at')
      .eq('id', id)
      .maybeSingle()
    ...
    const tarih = kayit.created_at
      ? calismaBelgesiTarihFormat(new Date(kayit.created_at))
      : calismaBelgesiTarihFormat()

    const alanlar = calismaBelgesiBelgeAlanlari(kayit, tarih)
      ...
      sections: [
        {
          ...
          children: [
            ...
            new Paragraph({
              ...
              children: [run(CALISMA_BELGESI_MAKAM, { bold: true })],
            }),
            new Paragraph({
              alignment: AlignmentType.CENTER,
              spacing: { after: 600 },
              children: [run(CALISMA_BELGESI_BIRIM)],
            }),
            new Paragraph({
    ...
    const filename = `Calisma_Belgesi_Talebi_${safeKey}.docx`
    ...
    return new NextResponse(new Uint8Array(buf), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Calisma_Belgesi_Talebi.docx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('CALISMA_BELGESI_WORD_HATA', err)
