...
/** Veri içeren hücre veya birleşimin köşe hücresinde veri varsa kenarlık (birleşik blokta tüm hücrelere). */
...
export async function GET(req: Request) {
  try {
    ...
    const rows: (string | number)[][] = [
      padRow(COL_LAST + 1, ['Statüye Göre Cinsiyet Raporu']),
      padRow(COL_LAST + 1, [`Yıl: ${yil} · Sekme: ${label}`]),
      padRow(COL_LAST + 1, [`Anlık görüntü tarihi: ${sonGunuMetin(D)}`]),
      padRow(COL_LAST + 1, ['']),
      padRow(COL_LAST + 1, ['Sıra No', 'Statü', 'Kadın', 'Erkek', 'Toplam']),
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Statü Cinsiyet')
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })
    const filename = `Statuye_Gore_Cinsiyet_${yil}_${label}.xlsx`
    const encodedFilename = encodeURIComponent(filename)

    return new NextResponse(buf, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Statuye_Gore_Cinsiyet.xlsx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('STATU_CINSIYET_EXCEL_HATA', err)
