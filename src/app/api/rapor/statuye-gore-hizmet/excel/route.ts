...
export async function GET(req: Request) {
  try {
    ...
    const snap = statuHizmetSnapshot({ D, tanimStatuler, kadro, calisanBySicil })
    ...
    const kolonToplam = kolonToplamlari(snap.satirlar, snap.kolonlar.length)
    const genelToplam = kolonToplam.reduce((n, v) => n + v, 0)
    const COL_LAST = 2 + snap.kolonlar.length

    const rows: (string | number)[][] = [
      padRow(COL_LAST + 1, ['Statüye Göre Hizmet Raporu']),
      ...
      ...snap.satirlar.map((s, i) =>
        padRow(COL_LAST + 1, [i + 1, s.statuEtiket, ...s.sayilar, s.sayilar.reduce((n, v) => n + v, 0)]),
      ),
    ...
    ws['!cols'] = [{ wch: 8 }, { wch: exactExcelWch(15) }, ...snap.kolonlar.map(() => ({ wch: 10 })), { wch: 8 }]
    ws['!rows'] = []
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Statü Hizmet')
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })
    const filename = `Statuye_Gore_Hizmet_${yil}_${label}.xlsx`
    const encodedFilename = encodeURIComponent(filename)
    return new NextResponse(buf, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Statuye_Gore_Hizmet.xlsx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('STATU_HIZMET_EXCEL_HATA', err)
