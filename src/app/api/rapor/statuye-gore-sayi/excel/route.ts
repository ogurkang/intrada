...
import { statuSayiSnapshot } from '@/lib/rapor-statuye-gore-sayi'
...
export async function GET(req: Request) {
  try {
    ...
    const genelToplam = snap.satirlar.reduce((n, s) => n + s.sayi, 0)

    const rows: (string | number)[][] = [
      padRow(COL_LAST + 1, ['Statüye Göre Sayı Durumu Raporu']),
      padRow(COL_LAST + 1, [`Yıl: ${yil} · Sekme: ${label}`]),
      padRow(COL_LAST + 1, [`Anlık görüntü tarihi: ${sonGunuMetin(D)}`]),
      padRow(COL_LAST + 1, ['']),
      padRow(COL_LAST + 1, ['Sıra No', 'Statü', 'Sayı']),
      ...snap.satirlar.map((s, i) => padRow(COL_LAST + 1, [i + 1, s.statuEtiket, s.sayi])),
      padRow(COL_LAST + 1, ['Toplam', '', genelToplam]),
      padRow(COL_LAST + 1, ['']),
    ...
    ws['!cols'] = [{ wch: 8 }, { wch: exactExcelWch(65) }, { wch: 12 }]
    ...
    for (let r = 0; r <= range.e.r; r++) {
      for (let c = 0; c <= range.e.c; c++) {
        ...
        else if (sayiCol && r >= headerRow && r <= totalRow) horizontal = 'center'
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Statü Sayı')
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })
    const filename = `Statuye_Gore_Sayi_Durumu_${yil}_${label}.xlsx`
    const encodedFilename = encodeURIComponent(filename)

    return new NextResponse(buf, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Statuye_Gore_Sayi_Durumu.xlsx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('STATU_SAYI_EXCEL_HATA', err)
