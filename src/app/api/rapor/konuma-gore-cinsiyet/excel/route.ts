...
/** Veri tablosu: A–E (Sıra No … Toplam) */
const COL_LAST = 4
...
export async function GET(req: Request) {
  try {
    ...
    const kTop = snap.satirlar.reduce((n, s) => n + s.kadin, 0)
    const eTop = snap.satirlar.reduce((n, s) => n + s.erkek, 0)
    const tTop = snap.satirlar.reduce((n, s) => n + s.kadin + s.erkek, 0)

    const rows: (string | number)[][] = [
      padRow(COL_LAST + 1, ['Konuma Göre Cinsiyet Raporu']),
      padRow(COL_LAST + 1, [`Yıl: ${yil} · Sekme: ${label}`]),
      padRow(COL_LAST + 1, [`Anlık görüntü tarihi: ${sonGunuMetin(D)}`]),
      padRow(COL_LAST + 1, ['']),
      padRow(COL_LAST + 1, ['Sıra No', 'Konum', 'Kadın', 'Erkek', 'Toplam']),
      ...snap.satirlar.map((s, i) =>
        padRow(COL_LAST + 1, [i + 1, s.statuEtiket, s.kadin, s.erkek, s.kadin + s.erkek]),
      ),
      padRow(COL_LAST + 1, ['Toplam', '', kTop, eTop, tTop]),
      padRow(COL_LAST + 1, ['']),
    ]
    ...
    if (snap.konumAtanmamisListe.length > 0) {
      const titleRow = rows.length
      rows.push(padRow(COL_LAST + 1, ['Konum atanmamış personel']))
      const contentRow = rows.length
      rows.push(padRow(COL_LAST + 1, [snap.konumAtanmamisListe.join(', ')]))
      rows.push(padRow(COL_LAST + 1, ['']))

      merges.push({ s: { r: titleRow, c: 0 }, e: { r: titleRow, c: COL_LAST } })
      merges.push({ s: { r: contentRow, c: 0 }, e: { r: contentRow, c: COL_LAST } })
      nameBlockRows.add(titleRow)
      nameBlockRows.add(contentRow)
      nameListRows.add(contentRow)
      tallRows.add(contentRow)
    }
    ...
    ws['!merges'] = merges
    ws['!cols'] = [{ wch: 8 }, { wch: 45 }, { wch: 10 }, { wch: 10 }, { wch: 10 }]
    ws['!rows'] = []
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Konum Cinsiyet')
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })
    const filename = `Konuma_Gore_Cinsiyet_${yil}_${label}.xlsx`
    const encodedFilename = encodeURIComponent(filename)

    return new NextResponse(buf, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Konuma_Gore_Cinsiyet.xlsx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('KONUM_CINSIYET_EXCEL_HATA', err)
