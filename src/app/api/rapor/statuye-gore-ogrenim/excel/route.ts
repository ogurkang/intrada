...
export async function GET(req: Request) {
  try {
    ...
    const [
      ...
      supabase.from('tanim_ogrenim').select('isim, aktif').eq('aktif', true),
    ...
    const tanimOgrenimIsimler = sortOgrenimIsimListesi((ogrenimTanimRaw ?? []).map(o => o.isim).filter(Boolean))
    ...
    const snap = statuOgrenimSnapshot({
      D,
      tanimStatuler,
      tanimOgrenimIsimler,
      kadro,
    ...
    const rows: (string | number)[][] = [
      padRow(COL_LAST + 1, ['Statüye Göre Öğrenim Durumu Raporu']),
      padRow(COL_LAST + 1, [`Yıl: ${yil} · Sekme: ${label}`]),
      padRow(COL_LAST + 1, [`Anlık görüntü tarihi: ${sonGunuMetin(D)}`]),
      padRow(COL_LAST + 1, ['']),
      padRow(COL_LAST + 1, ['Sıra No', 'Statü', ...snap.kolonlar, 'Toplam']),
    ...
    if (snap.belirtilmemisListe.length > 0) {
      const titleRow = rows.length
      rows.push(padRow(COL_LAST + 1, ['Öğrenim durumu belirtilmemiş personel']))
      const contentRow = rows.length
      rows.push(padRow(COL_LAST + 1, [snap.belirtilmemisListe.join(', ')]))
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Statü Öğrenim')
    const buf = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' })
    const filename = `Statuye_Gore_Ogrenim_${yil}_${label}.xlsx`
    const encodedFilename = encodeURIComponent(filename)

    return new NextResponse(buf, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="Statuye_Gore_Ogrenim.xlsx"; filename*=UTF-8''${encodedFilename}`,
      },
    })
  } catch (err) {
    console.error('STATU_OGRENIM_EXCEL_HATA', err)
