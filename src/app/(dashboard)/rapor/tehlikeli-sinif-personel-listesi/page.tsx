...
export default async function TehlikeliSinifPersonelListesiPage({ searchParams }: { searchParams: Promise<{ y?: string; p?: string }> }) {
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-800 mt-1">Tehlikeli Sınıfına Göre Personel Listesi</h1>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/api/rapor/tehlikeli-sinif-personel-listesi/excel?y=${yil}&p=${p}`} className="bg-emerald-700 text-white text-sm px-4 py-2 rounded-lg">Excel İndir ({label})</Link>
      ...
      <div className="bg-white border rounded-xl overflow-x-auto">
        <table className="w-full text-sm"><thead><tr className="bg-slate-50 border-b"><th className="px-3 py-2 text-center">Sıra No</th><th className="px-3 py-2 text-left">Sicil No</th><th className="px-3 py-2 text-left">Adı Soyadı</th><th className="px-3 py-2 text-left">Müdürlük</th></tr></thead><tbody className="divide-y">{satirlar.map((r, i) => <tr key={`${r.sicil_no}-${i}`}><td className="px-3 py-2 text-center">{i + 1}</td><td className="px-3 py-2">{r.sicil_no}</td><td className="px-3 py-2">{r.ad_soyad}</td><td className="px-3 py-2">{r.mudurluk}</td></tr>)}</tbody></table>
      </div>
    </div>
  )
}
