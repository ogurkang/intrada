...
const OGRENIM_ACIKLAMA =
  'Kadro: varsayılan «Lisans» ise aynı sicildeki tüm öğrenim satırlarına bakılır (pasif kayıtlar dahil); Lisans üstü en yüksek düzey (ör. Doktora varsa doktora, yoksa yüksek lisans) raporda kullanılır. Varsayılan Lisans değilse veya varsayılan yoksa önce varsayılan (yoksa aktif) kullanılır. Firma: karttaki öğrenim alanı. Tanımlar > Öğrenim ile eşleşmeyen «Belirtilmemiş».'

const OGRENIM_ALT_NOT =
  'Gelenler / Ayrılanlar: seçili dönemde kuruma giriş veya işe başlama; ayrılış tarihleri diğer raporlarla aynı mantıktadır.'

export default async function StatuyeGoreOgrenimPage({
  searchParams,
}: {
  ...
  const tanimOgrenimIsimler = sortOgrenimIsimListesi(
    (ogrenimTanimRaw ?? []).map(o => o.isim).filter(Boolean),
  )
  ...
  const tabs: StatuyeGoreMatrisTabVerisi[] = periyotlar.map(p => {
    ...
    return {
      ...
      gelenler,
      ayrilanlar,
      belirtilmemisListe: snap.belirtilmemisListe,
    }
  })
