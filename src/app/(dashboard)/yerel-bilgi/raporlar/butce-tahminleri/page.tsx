...
export default async function ButceTahminleriRaporPage({ searchParams }: Props) {
  ...
  const [{ data: giderDefs }, { data: gelirDefs }, { data: islemRaw }, { data: mudRow }] = await Promise.all([
    ...
    mudId != null ? supabase.from('yerel_bilgi_butce_tahmin_islem').select('*').eq('mudurluk_id', mudId) : Promise.resolve({ data: [] }),
  ...
  const excelHref =
    mudId != null
      ? `/api/yerel-bilgi/raporlar/butce-tahminleri/excel?mudurluk_id=${mudId}`
      : null
  const excelBtn =
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">{yil} Yılı Bütçe Tahminleri Raporu</h1>
      ...
      {isAdmin && mudurlukler.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <YerelBilgiMudurlukSecici
            ...
            basePath="/yerel-bilgi/raporlar/butce-tahminleri"
          />
          {mudId == null && (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-4 py-3 bg-slate-50 border-b font-semibold text-slate-700">Bütçe Gider Türü (Tahmin)</div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-4 py-3 bg-slate-50 border-b font-semibold text-slate-700">Bütçe Gelir Türü (Tahmin)</div>
          <table className="w-full text-sm">
            <tbody className="divide-y divide-slate-100">
              {Array.from({ length: maxSatir }, (_, i) => {
                const k = gelirRows[i]
