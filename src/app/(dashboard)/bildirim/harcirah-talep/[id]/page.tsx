...
export default async function HarcirahTalepDetayPage({ params }: Props) {
  ...
  const { data: kayit } = await (supabase as any)
    ...
    .select(
      'id, sicil_no, ad_soyad, tckn, adres, geldigi_kurum, nakil_tarihi, created_at, created_by_email',
    )
    .eq('id', id)
    .maybeSingle()
  ...
  const satirlar: { etiket: string; deger: string }[] = [
    ...
    { etiket: 'Adres', deger: kayit.adres ?? '—' },
    { etiket: 'Geldiği Kurum', deger: kayit.geldigi_kurum ?? '—' },
    { etiket: 'Nakil Tarihi', deger: harcirahTalepTarihGoster(kayit.nakil_tarihi) },
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-800">Harcırah Talep Bildirimi Detayı</h1>
        </div>
        <a
          href={`/api/bildirim/harcirah-talep/word?id=${kayit.id}`}
