...
export default async function OkulaUyumIzniDetayPage({ params }: Props) {
  ...
  const { data: kayit } = await (supabase as any)
    ...
    .select(
      'id, sicil_no, ad_soyad, tckn, unvan, mudurluk, ogrenci_ad_soyad, baslayacagi_sinif, created_at, created_by_email',
    )
    .eq('id', id)
    .maybeSingle()
  ...
  const satirlar: { etiket: string; deger: string }[] = [
    ...
    { etiket: 'Öğrenci Adı Soyadı', deger: kayit.ogrenci_ad_soyad ?? '—' },
    { etiket: 'Başlayacağı Sınıf', deger: kayit.baslayacagi_sinif ?? '—' },
