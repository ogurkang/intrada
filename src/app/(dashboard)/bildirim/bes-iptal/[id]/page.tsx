...
export default async function BesIptalDetayPage({ params }: Props) {
  ...
  const { data: kayit } = await (supabase as any)
    ...
    .select('id, sicil_no, ad_soyad, tckn, created_at, created_by_email')
    .eq('id', id)
    .maybeSingle()
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-800">BES İptal Talebi Detayı</h1>
        </div>
        <a
          href={`/api/bildirim/bes-iptal/word?id=${kayit.id}`}
