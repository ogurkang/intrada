...
export default async function MehilIzniDetayPage({ params }: Props) {
  ...
  const { data: kayit } = await (supabase as any)
    ...
    .select(
      'id, sicil_no, ad_soyad, tckn, geldigi_kurum, nakil_tarihi, mehil_baslangic_tarihi, mehil_bitis_tarihi, created_at, created_by_email',
    )
    .eq('id', id)
    .maybeSingle()
  ...
  const satirlar: { etiket: string; deger: string }[] = [
    ...
    { etiket: 'Geldiği Kurum', deger: kayit.geldigi_kurum ?? '—' },
    { etiket: 'Nakil Tarihi', deger: mehilIzniTarihGoster(kayit.nakil_tarihi) },
    {
      etiket: 'Mehil Aralığı',
      deger: `${mehilIzniTarihGoster(kayit.mehil_baslangic_tarihi)} – ${mehilIzniTarihGoster(kayit.mehil_bitis_tarihi)}`,
    },
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-800">Mehil İzni Bildirimi Detayı</h1>
        </div>
        <a
