...
export interface IzyDetayData {
  donem:    { id: number; donem_adi: string | null; baslangic_tarihi: string; bitis_tarihi: string }
  aday:     IzyDetayIzin[]
  islenecek: IzyDetayIzin[]
}

export async function izyDetayYukle(donem_id: number): Promise<IzyDetayData | { hata: string }> {
  ...
  // Zabıta Müdürlüğü'ne ait TÜM izinler — İptal hariç; tür: Yıllık, Ölüm, Evlilik, Babalık, Mehil, Mazeret, İdari, DÖÇ, DSÇ
  ...
  const tumIzinler: IzyDetayIzin[] = (izinRaw ?? [])
  ...
  const islenecek: IzyDetayIzin[] = []
  const aday: IzyDetayIzin[] = []
...
export async function izySecimleriKaydet(donem_id: number, siraNoList: string[]): Promise<{ hata?: string }> {
  const supabase = await createClient()
  await supabase.from('izinli_zabitalar_yeni_secim').delete().eq('donem_id', donem_id)
  if (siraNoList.length > 0) {
    const { error } = await supabase.from('izinli_zabitalar_yeni_secim').insert(
      siraNoList.map(izin_sira_no => ({ donem_id, izin_sira_no, dahil: true })) as never[]
    )
    if (error) return { hata: error.message }
  }
  revalidatePath(`/kesintiler/izy/${donem_id}`)
  revalidatePath('/kesintiler/izy')
  return {}
}
