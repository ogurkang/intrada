...
import IzyDetayClient from '@/components/kesintiler/IzyDetayClient'

interface Props {
  params: Promise<{ donem_id: string }>
}

export default async function IzyDetayPage({ params }: Props) {
    ...
    <div>
      <nav className="flex items-center justify-between gap-3 mb-6">
        <span className="flex items-center gap-2 text-sm text-slate-500">
          <Link href="/kesintiler/izy" className="hover:text-slate-800 transition-colors">
            İzinli Zabıtalar (IZY)
          </Link>
