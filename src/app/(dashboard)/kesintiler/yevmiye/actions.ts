...
const PATH = '/kesintiler/yevmiye'
const REF_TABLE = 'yevmiye_donem'
const MODUL = 'YEV'
...
export async function yevmiyeDonemEkle(fd: FormData): Promise<{ hata?: string }> {
  ...
  const { data: inserted, error } = await supabase.from('yevmiye_donem').insert(payload).select('id').single()
...
export async function yevmiyeDonemGuncelle(id: number, fd: FormData): Promise<{ hata?: string }> {
  const supabase = await createClient()
  ...
  const { error } = await supabase.from('yevmiye_donem').update(payload).eq('id', id)
...
export async function yevmiyeDonemKapat(id: number): Promise<{ hata?: string }> {
  const supabase = await createClient()
  const onceki = await fetchKesintiDonemAuditRow(supabase, REF_TABLE, id)
  const { error } = await supabase.from('yevmiye_donem').update({ durum: 'Kapalı' }).eq('id', id)
  if (error) return { hata: error.message }
  await writeKesintiDonemAuditLogSafe(supabase, {
    refTable: REF_TABLE, modul: MODUL, donemId: id,
...
export async function yevmiyeDonemAc(id: number): Promise<{ hata?: string }> {
  const supabase = await createClient()
  const onceki = await fetchKesintiDonemAuditRow(supabase, REF_TABLE, id)
  const { error } = await supabase.from('yevmiye_donem').update({ durum: 'Açık' }).eq('id', id)
  if (error) return { hata: error.message }
  await writeKesintiDonemAuditLogSafe(supabase, {
    refTable: REF_TABLE, modul: MODUL, donemId: id,
